 ...
 */
public class StaTimeIntervalExpression extends TimeExpression {
    ...
    public StaTimeIntervalExpression(Map<String, Field<Timestamp>> expressions) {
        this.start = expressions.get(KEY_TIME_INTERVAL_START);
        this.end = expressions.get(KEY_TIME_INTERVAL_END);
    }

    public StaTimeIntervalExpression(Field<Timestamp> start, Field<Timestamp> end) {
        this.start = start;
        this.end = end;
    }

    public StaTimeIntervalExpression(Timestamp start, Timestamp end) {
        this.start = new StaDateTimeExpression(DSL.val(start));
        this.end = StaDateTimeExpression.createDateTimeExpression(ConstantImpl.create(end));
    }

    public Field<Timestamp> getStart() {
        return start;
    }

    public Field<Timestamp> getEnd() {
        return end;
    }
    ...
    private Field<?> specificOp(String op, StaTimeIntervalExpression other) {
        switch (op) {
            case "-":
                // We calculate with the start time and return a duration.
    ...
    @Override
    public Field<Object> simpleOp(String op, Field<?> other) {
